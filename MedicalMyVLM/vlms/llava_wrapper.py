...
class LLaVAWrapper(VLMWrapper):
    ...
    def __init__(self, device: str = 'cuda', torch_dtype: torch.dtype = torch.float16):
        ...
        self.model_path = 'microsoft/llava-med-v1.5-mistral-7b' #omer
    ...
    def prepare_inputs(self, image: Image.Image, prompt: str, target: str) -> LLaVAInput:
        ...
        attention_mask = input_ids.ne(self.processor.tokenizer.pad_token_id) # omer
        # attention_mask = input_ids.ne(self.processor.tokenizer.eos_token_id)
    ...
    def generate(self, inputs: Dict[str, Any], concept_signals: Optional[torch.Tensor] = None) -> Union[str, List[str]]:
        ...
        with torch.cuda.amp.autocast():
            output = self.model.generate(inputs=inputs['input_ids'],
                                         pad_token_id=self.processor.tokenizer.pad_token_id, # omer
    ...
    def _prepare_inputs_for_generation(self,
        ...
        input_ids_padded = torch.nn.utils.rnn.pad_sequence(
            inputs_no_target,
            batch_first=True,
            padding_value=self.processor.tokenizer.pad_token_id # omer
        ...
        generation_inputs = dict(
            ...
            # attention_mask=input_ids_padded.ne(self.processor.tokenizer.eos_token_id),
