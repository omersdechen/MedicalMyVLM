...
class MyLLaVA(MyVLM):
    ...
    def validate(self, val_dataloader, temperature: float = 0.2, top_p: float = 0.7):
        ...
        keywords = [stop_str]
        outputs = []
        if self.vlm.processor.tokenizer.pad_token_id is None:
            self.vlm.processor.tokenizer.pad_token_id = self.vlm.processor.tokenizer.eos_token_id
        ...
        for val_batch in tqdm(val_dataloader):
            ...
            self.vlm.model = self.vlm.model.bfloat16() # omer
            ...
            generated_ids = self.vlm.model.generate(inputs=input_ids,
                                                    ...
                                                    pad_token_id=self.vlm.processor.tokenizer.pad_token_id,
                                                    stopping_criteria=[stopping_criteria],
                                                    do_sample=True,
                                                    temperature=temperature,
                                                    top_p=top_p,
                                                    max_new_tokens=512)
            # self.vlm.model = self.vlm.model.float16() # omer
    ...
    def _collate_func(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        joined_batch['input_ids'] = torch.nn.utils.rnn.pad_sequence(
            ...
            batch_first=True,
            padding_value=self.vlm.processor.tokenizer.pad_token_id # omer
            # padding_value=self.vlm.processor.tokenizer.eos_token_id
        )
        joined_batch['attention_mask'] = joined_batch['input_ids'].ne(self.vlm.processor.tokenizer.pad_token_id) # omer
        # joined_batch['attention_mask'] = joined_batch['input_ids'].ne(self.vlm.processor.tokenizer.eos_token_id)
